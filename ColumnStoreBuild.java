 ...
 * Years are in the 2000s: fullYear = 2000 + YY.
 */
public class ColumnStoreBuild {
    ...
    /** Maps month abbreviations (e.g. "Jan") to their numeric equivalent (1-12). */
     ...
     */
    public ColumnStoreBuild(String inputCsvPath, String colDir) {
    ...
    /**
     * Parses the input CSV row by row and writes each field to its respective column file.
     * Rows are skipped (with a warning) if they detect any issues not following the schema.
     * The READY marker is written after all writers are closed.
     */
    private void writeColumnFiles() {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                // parts[2] = flat_type    (skipped, not needed for output)
                String block     = parts[3].trim();
                // parts[4] = street_name  (skipped, not needed for output)
                // parts[5] = storey_range (skipped, not needed for output)
